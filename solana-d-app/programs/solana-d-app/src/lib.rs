...
const MAX_COMMUNITIES_PER_USER: usize = 100;
// Maximum number of surveys per community
const MAX_SURVEYS_PER_COMMUNITY: usize = 100;
...
declare_id!("Ho1P3APYbSz3DUZyjNiezxuVkGinLB9vkqAfLBfVM8Cm");
...
pub mod solana_d_app {
    ...
    pub fn register_user(ctx: Context<RegisterUser>) -> Result<()> {
        ...
        // Add the 'all' community name to the user's communities
        user_account.communities.push("all".to_string());
        msg!("Registering user: {}", ctx.accounts.authority.key());
        msg!("User PDA address: {}", ctx.accounts.user_account.key());
        msg!("Added 'all' community to user's communities");
        Ok(())
    }
    ...
    pub fn create_community(ctx: Context<CreateCommunity>, name: String) -> Result<()> {
        ...
        community_account.authority = ctx.accounts.authority.key();
        community_account.surveys = Vec::new();
        // Add the community name to the creator's communities
        user_account.communities.push(name.clone());
    ...
    pub fn join_community(ctx: Context<JoinCommunity>) -> Result<()> {
        ...
        if user_account.communities.iter().any(|n| n == &community_account.name) {
        ...
        // Add the community name to the user's communities
        user_account.communities.push(community_account.name.clone());
    ...
    pub fn create_survey(
        ...
        let community_account = &ctx.accounts.community_account;
        let is_member = user_account.communities.iter().any(|n| n == &community_account.name);
    ...
    pub fn exit_community(ctx: Context<ExitCommunity>) -> Result<()> {
        let user_account = &mut ctx.accounts.user_account;
        let community_account = &ctx.accounts.community_account;
        let authority_key = ctx.accounts.authority.key();

        // The authority of the community cannot exit
        require!(community_account.authority != authority_key, ErrorCode::CommunityAuthorityCannotExit);

        // Check if user is a member of the community
        let idx = user_account.communities.iter().position(|n| n == &community_account.name);
        require!(idx.is_some(), ErrorCode::NotMemberOfCommunity);

        // Remove the community from the user's communities
        user_account.communities.remove(idx.unwrap());
        msg!("User {} exited community '{}'", authority_key, community_account.name);
        Ok(())
    }
}
...
pub struct RegisterUser<'info> {
    #[account(
        init,
        payer = authority,
        space = 8 + 32 + 4 + (MAX_COMMUNITIES_PER_USER * (4 + MAX_SURVEY_TITLE_LENGTH)), // Max community names (up to 30 chars each)
...
pub struct UserAccount {
    ...
    pub communities: Vec<String>, // Store community names only
}
